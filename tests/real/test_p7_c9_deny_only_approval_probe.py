"""P7.C9 deny-only approval-probe preparation.
...
REAL_PROBE_LATCH = Path("/root/.codexcontrol/p7c9-deny-only-approval-probe-ledger.json")
REAL_PROBE_RESULT = Path("/root/.codexcontrol/p7c9-deny-only-approval-probe-result.json")
REAL_PROBE_OUTCOME = Path("/root/.codexcontrol/p7c9-deny-only-approval-probe-outcome.json")
...
P7C9_CANDIDATE_SLEEP_SECONDS = 30.0
...
# P7.C9 frozen real-mode authorities.  Synthetic tests may inject smaller values.
PROFILE_ID = "p7c9-fresh-probe"
...
P7C9_KNOWN_NAMED_STARTUP_BOUND_SECONDS = 24.0
KNOWN_NAMED_STARTUP_BOUND_SECONDS = P7C9_KNOWN_NAMED_STARTUP_BOUND_SECONDS
P7C9_RUNTIME_ACQUIRE_NAMED_STAGE_MARGIN_SECONDS = 15.0
P7C9_OBSERVATION_MARGIN_SECONDS = PROBE_OBSERVATION_MARGIN_SECONDS
P7C9_OBSERVATION_TIMEOUT_SECONDS = PROBE_OBSERVATION_TIMEOUT
P7C9_MODEL_LIST_TIMEOUT_SECONDS = PROBE_MODEL_LIST_TIMEOUT
P7C9_THREAD_START_TIMEOUT_SECONDS = PROBE_THREAD_START_TIMEOUT
P7C9_TURN_START_TIMEOUT_SECONDS = PROBE_TURN_START_TIMEOUT
P7C9_APPROVAL_RESPONSE_TIMEOUT_SECONDS = PROBE_APPROVAL_RESPONSE_TIMEOUT
P7C9_RUNTIME_SHUTDOWN_TIMEOUT_SECONDS = PROBE_RUNTIME_SHUTDOWN_TIMEOUT
P7C9_CHILD_RESULT_TIMEOUT_SECONDS = PROBE_CHILD_RESULT_TIMEOUT
P7C9_TERM_GRACE_SECONDS = PROBE_TERM_GRACE_SECONDS
P7C9_KILL_GRACE_SECONDS = PROBE_KILL_GRACE_SECONDS
P7C9_INTERNAL_WORST_CASE_SECONDS = PROBE_INTERNAL_WORST_CASE_SECONDS
P7C9_NORMAL_PATH_INTERNAL_WORST_CASE_SECONDS = NORMAL_PATH_INTERNAL_WORST_CASE_SECONDS
P7C9_FAILED_PROVISION_INTERNAL_WORST_CASE_SECONDS = FAILED_PROVISION_INTERNAL_WORST_CASE_SECONDS
P7C9_FAILED_ACQUIRE_INTERNAL_WORST_CASE_SECONDS = FAILED_ACQUIRE_INTERNAL_WORST_CASE_SECONDS
P7C9_WATCHDOG_MARGIN_SECONDS = PROBE_WATCHDOG_MARGIN_SECONDS
P7C9_WATCHDOG_HARD_DEADLINE_SECONDS = PROBE_WATCHDOG_HARD_DEADLINE
...
def synthetic_state_root_authority(run: "FreshProbeRun") -> tuple[CodexProfile, IsolationPathAuthority]:
    ...
    profile = CodexProfile(
        "p7c9-fresh-probe", "/root/.codex_second", "P7.C9 fresh disposable", str(run.isolated_state),
    )
...
class FreshProbeRun:
    ...
    def skeleton(cls, parent: Path) -> "FreshProbeRun":
        """Create only the P7.C9-owned run skeleton; state is production-owned."""
        root = parent / ("codexcontrol-p7c9-probe-" + secrets.token_hex(16))
        root.mkdir(mode=0o700)
        state_parent = root / "state-parent"
        isolated = state_parent / "p7c9-isolated-state"
...
def validate_future_source_authority(repository: Path = Path("/opt/codex-control")) -> tuple[str, str]:
    ...
    if not expected_head or not expected_tree or len(expected_head) != 40 or len(expected_tree) != 40:
        raise RuntimeError("P7C9_SOURCE_AUTHORITY_UNSET")
    if repository != Path("/opt/codex-control") or repository.resolve() != Path("/opt/codex-control"):
        raise RuntimeError("P7C9_CANONICAL_REPOSITORY_REQUIRED")
    try:
        ...
        raise RuntimeError("P7C9_SOURCE_GATE_UNAVAILABLE") from error
    if head != expected_head or tree != expected_tree or clean:
        raise RuntimeError("P7C9_SOURCE_GATE_MISMATCH")
    return head, tree
...
def require_parent_execution_authorities_absent(
    ...
    if any(_authority_path_present(path) for path in (latch, normal_result, execution_outcome)):
        raise RuntimeError("P7C9_PREEXISTING_PARENT_AUTHORITY")
...
def preflight_future_probe_boundaries(run: FreshProbeRun) -> None:
    ...
    for path in (run.root, run.state_parent, run.isolated_state, run.sqlite, run.logs, run.controller, run.workdir, run.sentinel):
        if any(path == other or other in path.parents or path in other.parents for other in protected):
            raise RuntimeError("P7C9_PROBE_PATH_OVERLAP")
    if not _private_directory(run.root) or not _private_directory(run.workdir):
        raise RuntimeError("P7C9_PROBE_ROOT_AUTHORITY_INVALID")
    if run.sentinel.exists():
        raise RuntimeError("P7C9_PREEXISTING_PROBE_ARTIFACT")
...
def require_dedicated_future_process_group() -> tuple[int, int, int]:
    ...
    if pid != pgid or pid != sid or pgid <= 1 or pgid == parent_pgid:
        raise RuntimeError("P7C9_DEDICATED_PROCESS_GROUP_REQUIRED")
    return pid, pgid, sid
...
async def future_real_deny_only_approval_probe(run_parent: Path | None = None) -> dict[str, Any]:
    ...
    if os.environ.get(AUTHORIZED_ENV) != AUTHORIZED_ENV:
        raise unittest.SkipTest("P7C9_REAL_PROBE_GATE_DISABLED")
    ...
    if not _private_directory(REAL_PROBE_LATCH.parent):
        raise RuntimeError("P7C9_LATCH_PARENT_INVALID")
    if REAL_PROBE_LATCH.exists() or REAL_PROBE_RESULT.exists():
        raise RuntimeError("P7C9_ONE_SHOT_ALREADY_CONSUMED")
    temporary_parent = run_parent or Path(tempfile.mkdtemp(prefix="codexcontrol-p7c9-real-", dir="/tmp"))
    ...
    profile = CodexProfile("p7c9-fresh-probe", "/root/.codex_second", "P7.C9 fresh disposable", str(run.isolated_state))
    authority = IsolationPathAuthority(
        (profile,), controller_db_root=str(run.controller), repository_root="/opt/codex-control",
        protected_roots=("/root/.codexcontrol",),
    )
    ...
    try:
        ...
        acquire_observer = RuntimeAcquireObserver(
            timeout=P7C9_RUNTIME_ACQUIRE_TIMEOUT_SECONDS,
            cleanup_timeout=P7C9_RUNTIME_ACQUIRE_CLEANUP_TIMEOUT_SECONDS,
        )
        ...
        if len(defaults) != 1:
            raise RuntimeError("P7C9_MODEL_DEFAULT_AMBIGUOUS")
        model = defaults[0]
        ...
        if thread.status is not ThreadOperationStatus.START_CONFIRMED or thread.binding is None:
            raise RuntimeError("P7C9_THREAD_START_NOT_CONFIRMED")
        ...
        if turn.status is not TurnStartStatus.CONFIRMED or turn.binding is None:
            raise RuntimeError("P7C9_PRIMARY_TURN_NOT_CONFIRMED")
...
def scan_fresh_run_boundary(
    ...
    fixed = {"state-parent", "state-parent/p7c9-isolated-state", "controller", "workdir", "outside-workdir-sentinel",
    ...
    for entry in run.root.iterdir():
        ...
        if relative == CHILD_RESULT_FILENAME and phase == PARENT_POST_QUIESCENCE:
            continue
        if relative == "state-parent/p7c9-isolated-state":
            continue
...
def run_future_real_probe_parent() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("P7C9_ONE_SHOT_ALREADY_CONSUMED") from None
    child_parent = Path(tempfile.mkdtemp(prefix="codexcontrol-p7c9-parent-", dir="/tmp"))
    child = subprocess.Popen(
        [sys.executable, str(Path(__file__).resolve()), "--codexcontrol-p7c9-probe-child", str(child_parent)],
...
class RuntimeAcquireObserver:
    ...
    def __init__(
        self, *, timeout: float = P7C9_RUNTIME_ACQUIRE_TIMEOUT_SECONDS,
        cleanup_timeout: float = P7C9_RUNTIME_ACQUIRE_CLEANUP_TIMEOUT_SECONDS,
        cleanup_cancel_join: float = P7C9_RUNTIME_ACQUIRE_CLEANUP_CANCEL_JOIN_SECONDS,
    ) -> None:
...
class StateRootProvisionOfflineTests(unittest.IsolatedAsyncioTestCase):
    def _offline_fixture(self, root: Path) -> tuple[FreshProbeRun, CodexProfile, IsolationPathAuthority]:
        ...
        profile = CodexProfile("p7c9-fresh-probe", str(persistent), "offline", str(run.isolated_state))
        authority = IsolationPathAuthority(
    ...
    def test_production_provision_from_absent_root_and_immediate_validate(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-state-root-") as directory:
    ...
    def test_incomplete_manual_root_is_rejected_by_production_validate(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-state-negative-") as directory:
            run, profile, authority = self._offline_fixture(Path(directory))
    ...
    def test_second_provision_guard_fails_closed_and_identity_is_stable(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-state-once-") as directory:
            run, profile, authority = self._offline_fixture(Path(directory))
    ...
    async def test_each_provision_or_validate_failure_blocks_runtime_acquire(self) -> None:
        ...
        for name, provision, validate in cases:
            with self.subTest(name=name), tempfile.TemporaryDirectory(prefix="p7c9-state-failure-") as directory:
                root = Path(directory)
    ...
    def test_parent_provision_recovery_is_confirmed_only_for_exact_chronology(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-state-recovery-") as directory:
            root = Path(directory)
    ...
    def test_parent_provision_recovery_missing_unsafe_duplicate_conflicting_and_invalid_fail_closed(self) -> None:
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory(prefix="p7c9-state-recovery-negative-") as directory:
                root = Path(directory)
                run = FreshProbeRun.skeleton(root)
                journal = RecoveryJournal(run.probe_recovery, source_sha="a" * 40, source_tree="b" * 40)
                journal._append({"event": "GLOBAL_LATCH_RESERVED", "result": "YES"})
...
class RuntimeAcquireOfflineTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_safe_category_uses_separate_event_without_typeerror(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-cleanup-category-") as directory:
...
class Repair1AcquisitionAuthorityOfflineTests(unittest.TestCase):
    ...
    def test_success_and_failed_chronologies_are_final_result_authority(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-acquire-authority-") as directory:
            root = Path(directory)
    ...
    def test_parent_negative_matrix_is_never_normal_success(self) -> None:
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory(prefix="p7c9-acquire-negative-") as directory:
                root = Path(directory)
...
class DenyOnlyApprovalOfflineTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        self.directory = tempfile.TemporaryDirectory(prefix="p7c9-synthetic-")
...
class RaceOfflineTests(unittest.IsolatedAsyncioTestCase):
    async def _fixture(self, *, terminal_status: str = "COMPLETED") -> tuple[FreshProbeRun, SyntheticApprovalClient, DenyOnlyApprovalOperator, CodexApprovalBridge]:
        root = Path(tempfile.mkdtemp(prefix="p7c9-race-"))
...
class JournalOfflineTests(unittest.IsolatedAsyncioTestCase):
    async def test_journal_identity_continuity_matrix_is_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="p7c9-journal-continuity-") as directory:
            root = Path(directory)
    ...
    async def test_journal_initial_creation_is_exclusive_and_appends_keep_identity(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-journal-create-") as directory:
    ...
    async def test_journal_intent_failure_blocks_model_thread_turn_and_deny_effects(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-journal-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    async def test_journal_continuity_failure_does_not_dispatch_effect(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-journal-effect-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            journal = RecoveryJournal(run.probe_recovery, source_sha="a" * 40, source_tree="b" * 40)
            journal.path.unlink()
            calls = 0
    ...
    async def test_request_observed_is_durable_before_deny_intent(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-request-journal-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            journal = RecoveryJournal(run.probe_recovery, source_sha="a" * 40, source_tree="b" * 40)
            future = asyncio.get_running_loop().create_future()
    ...
    async def test_request_journal_failure_blocks_response_and_deny_attempt(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-request-journal-fail-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            journal = RecoveryJournal(run.probe_recovery, source_sha="a" * 40, source_tree="b" * 40)
...
class Repair3AuthorityOfflineTests(unittest.TestCase):
    ...
    def test_parent_writer_persists_parent_schema_and_child_writer_rejects_it(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-parent-writer-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_parent_recheck_detects_late_sentinel_and_workdir_drift(self) -> None:
        for mutation in ("sentinel", "workdir"):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory(prefix="p7c9-drift-") as directory:
                run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_unchanged_parent_recheck_is_drift_none(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-no-drift-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._child(run)
            parent = self._parent(child)
    ...
    def test_wire_and_adapter_results_are_separate_for_malformed_responses(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-stage-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            journal = RecoveryJournal(run.probe_recovery, source_sha="a" * 40, source_tree="b" * 40)
    ...
    def test_normal_child_result_requires_all_owned_observers_terminalized(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-owner-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            loop = asyncio.new_event_loop()
            future = loop.create_future()
            future.set_result("synthetic-turn")
...
class Repair4AuthorityOfflineTests(unittest.TestCase):
    ...
    def test_child_result_is_exact_parent_harness_authority_and_no_drift(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-happy-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child_boundary = scan_fresh_run_boundary(run, phase=CHILD_PRE_RESULT)
    ...
    def test_child_result_is_absent_at_child_pre_result_boundary(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-pre-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._child(run)
    ...
    def test_parent_boundary_rejects_unsafe_child_result_without_repairing_it(self) -> None:
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory(prefix="p7c9-r4-child-unsafe-") as directory:
                root = Path(directory)
                run = FreshProbeRun.materialize(root)
                child = self._child(run)
    ...
    def test_parent_boundary_rejects_child_result_path_replacement_and_read_mutation(self) -> None:
        ...
        for mutation in ("replace", "metadata"):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory(prefix="p7c9-r4-child-race-") as directory:
                root = Path(directory)
                run = FreshProbeRun.materialize(root)
                child = self._child(run)
                path = run.root / CHILD_RESULT_FILENAME
    ...
    def test_arbitrary_sibling_remains_unexpected_probe_mutation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-sibling-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._child(run)
            write_sanitized_result(run.root / CHILD_RESULT_FILENAME, child)
    ...
    def test_real_observation_authority_dominates_frozen_stimulus(self) -> None:
        self.assertEqual(P7C9_CANDIDATE_SLEEP_SECONDS, 30.0)
        self.assertGreater(PROBE_OBSERVATION_MARGIN_SECONDS, 0.0)
        self.assertGreater(
            PROBE_OBSERVATION_TIMEOUT,
            P7C9_CANDIDATE_SLEEP_SECONDS + PROBE_OBSERVATION_MARGIN_SECONDS,
        )
    ...
    def test_no_approval_terminal_branch_can_form_a_finite_child_result(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-no-approval-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._child(run)
            validate_child_result(child)
    ...
    def test_each_failure_class_persists_one_exact_outcome_authority(self) -> None:
        ...
        for execution_class, watchdog, child_class, extra, discovery in cases:
            with self.subTest(execution_class=execution_class), tempfile.TemporaryDirectory(prefix="p7c9-r4-outcome-") as directory:
                run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_normal_success_persists_final_result_before_confirmed_outcome(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-success-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._child(run)
            child_path = run.root / CHILD_RESULT_FILENAME
            write_sanitized_result(child_path, child)
    ...
    def test_failure_outcome_is_not_a_normal_parent_final_result(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-outcome-schema-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_parent_outcome_schema_rejects_raw_fields_and_arbitrary_classes(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-outcome-invalid-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_preexisting_parent_outcome_blocks_launch_authority(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r4-preexisting-") as directory:
            root = Path(directory)
...
class AuthorityAndBoundaryOfflineTests(unittest.TestCase):
    def test_fresh_run_layout_prompt_and_separate_authority(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-layout-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_probe_latch_is_exclusive_no_follow_and_blocks_preexisting(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-latch-") as directory:
    ...
    def test_boundary_no_sentinel_exact_sentinel_and_unexpected_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-boundary-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_boundary_unexpected_process_reference(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-process-boundary-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_exact_touch_authority_rejects_nonzero_hardlink_symlink_and_unsafe_mode(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-touch-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            run.sentinel.touch(mode=0o600)
    ...
    def test_unexpected_root_sibling_and_process_reference_are_command_owned(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-boundary-extra-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_sanitized_result_excludes_raw_wire_command(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-result-") as directory:
            root = Path(directory)
            run = FreshProbeRun.materialize(root)
    ...
    def test_child_result_has_no_process_group_final_claims(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-child-result-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            future = asyncio.new_event_loop().create_future()
            future.set_result("synthetic-turn-id")
            operator = DenyOnlyApprovalOperator(
    ...
    def test_parent_final_result_requires_quiescent_group_and_valid_child(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-parent-result-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            future = asyncio.new_event_loop().create_future()
            future.set_result("synthetic-turn-id")
            operator = DenyOnlyApprovalOperator(
...
class Repair5AuthorityOfflineTests(unittest.TestCase):
    ...
    def test_child_result_discovery_is_bounded_read_only_and_factful(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r5-discovery-") as directory:
            parent = Path(directory)
            self.assertEqual(
    ...
    def test_ambiguous_child_root_does_not_invent_absent_result_fact(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r5-ambiguous-") as directory:
            parent = Path(directory)
    ...
    def test_latch_presence_is_measured_before_and_after_failure(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r5-latch-") as directory:
...
class Repair6EffectLedgerOfflineTests(unittest.TestCase):
    ...
    def test_normal_child_projects_and_persists_authoritative_one_one_one_ledger(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r6-child-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            budget = synthetic_normal_budget()
            self.assertEqual(
    ...
    def test_normal_child_negative_matrix_rejects_lifecycle_hash_forbidden_and_allow_mutations(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r6-child-negative-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
    ...
    def test_normal_parent_requires_exact_one_one_one_ledger(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-r6-parent-") as directory:
            run = FreshProbeRun.materialize(Path(directory))
            child = self._normal_child(run)
            path = run.root / CHILD_RESULT_FILENAME
            write_sanitized_result(path, child)
...
class RawWireAuthorityOfflineTests(unittest.TestCase):
    def test_exclusive_root_only_record_and_bounded_reader(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-wire-") as directory:
            root = Path(directory)
            run = FreshProbeRun.materialize(root)
    ...
    def test_exact_schema_hash_identity_kind_sequence_and_size_rules(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-wire-schema-") as directory:
            root = Path(directory)
            run = FreshProbeRun.materialize(root)
            authority = WireCommandAuthority(run.wire_recovery)
    ...
    def test_symlink_hardlink_unsafe_mode_oversize_and_malformed_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-wire-unsafe-") as directory:
            root = Path(directory)
    ...
    def test_path_replacement_and_mutation_during_read_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="p7c9-wire-race-") as directory:
            root = Path(directory)
...
class P7C9StaticGateTests(unittest.TestCase):
    ...
    def test_future_unittest_uses_parent_launcher_not_inner_coroutine(self) -> None:
        source = inspect.getsource(P7C9DenyOnlyApprovalProbeAcceptance.test_future_real_deny_only_approval_probe)
        ...
        self.assertIn("--codexcontrol-p7c9-probe-child", parent_source)
    ...
    def test_failed_containment_has_one_task_and_no_unbounded_cancel_join(self) -> None:
        ...
        self.assertIn("P7C9_RUNTIME_ACQUIRE_CLEANUP_CANCEL_JOIN_SECONDS", inspect.getsource(RuntimeAcquireObserver))
    ...
    def test_acquisition_horizon_and_parent_outcome_fields_are_frozen(self) -> None:
        self.assertGreater(
            P7C9_RUNTIME_ACQUIRE_TIMEOUT_SECONDS,
            KNOWN_NAMED_STARTUP_BOUND_SECONDS + P7C9_RUNTIME_ACQUIRE_NAMED_STAGE_MARGIN_SECONDS,
        )
        self.assertEqual(P7C9_RUNTIME_ACQUIRE_CLEANUP_TIMEOUT_SECONDS, 12.0)
        outcome = make_parent_execution_outcome(
            execution_class=CHILD_RETURN_NONZERO, watchdog_status="PROCESS_COMPLETED",
            child_returncode_class=CHILD_RETURN_NONZERO, child_result_present=False,
            child_result_valid=False, parent_boundary_class="BOUNDARY_NOT_PROVED",
...
class P7C9DenyOnlyApprovalProbeAcceptance(unittest.IsolatedAsyncioTestCase):
    @unittest.skipUnless(
        ...
        "gated future P7.C9 deny-only approval probe",
    )
...
if __name__ == "__main__":
    if "--codexcontrol-p7c9-probe-child" in sys.argv:
        index = sys.argv.index("--codexcontrol-p7c9-probe-child")
