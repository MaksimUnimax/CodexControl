...
class FoundationTests(unittest.TestCase):
    def test_configuration_is_server_agnostic(self):
        config = parse_server_configuration({"server": {"server_id": "server-N", "display_name": "SERVER-N"}, "profiles": [{"profile_id": "alpha", "codex_home": "/safe/home", "display_name": "Alpha", "isolated_state_root": "/safe/state"}]})
    ...
    def test_running_turn_snapshot_is_not_retargeted(self):
        first = CodexSelection(CodexProfile("one", "/profile-one", "One", "/state-one"), "model-a", "high")
        binding = bind_dialogue("server-N", first, "thread-1")
        snapshot = capture_turn(binding)
        later = CodexSelection(CodexProfile("two", "/profile-two", "Two", "/state-two"), "model-b", "low")
    ...
    def test_profile_repr_redacts_home(self):
        self.assertNotIn("/private", repr(CodexProfile("p", "/private", "Profile", "/private-state")))

    def test_duplicate_profile_ids_are_rejected(self):
        with self.assertRaises(ValueError):
            parse_server_configuration({"server": {"server_id": "x", "display_name": "X"}, "profiles": [{"profile_id": "p", "codex_home": "/a", "display_name": "A", "isolated_state_root": "/sa"}, {"profile_id": "p", "codex_home": "/b", "display_name": "B", "isolated_state_root": "/sb"}]})
