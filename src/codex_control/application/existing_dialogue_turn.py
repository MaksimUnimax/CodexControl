...
class ExistingDialogueTurnService:
    ...
    async def _run_admitted(
        ...
        if (
            ...
            or current.thread_id != admitted.thread_id
        ...
        try:
            started = await self._turn_lifecycle.start_turn(
            ...
            return await self._finish(jobs, starting, claimed.dialogue, outcome, error_class, ())
        except Exception:
            return await self._finish(jobs, starting, claimed.dialogue, TurnTerminalOutcome.UNKNOWN, "CODEX_AMBIGUOUS", ())

        if not isinstance(started, TurnStartResult):
            return await self._finish(jobs, starting, claimed.dialogue, TurnTerminalOutcome.UNKNOWN, "CODEX_AMBIGUOUS", ())
        if started.status is TurnStartStatus.REJECTED:
            return await self._finish(jobs, starting, claimed.dialogue, TurnTerminalOutcome.FAILED, "CODEX_TURN_FAILED", ())
        ...
        try:
            ...
            return await self._running_bind_failure(jobs, starting, claimed.dialogue, error)
        if not isinstance(running, TurnJobRecord):
            return await self._running_bind_failure(jobs, starting, claimed.dialogue, _invariant())
        ...
        try:
            ...
            terminal = None
        outcome, error_class, messages = self._project_terminal(terminal, binding)
        return await self._finish(jobs, running, claimed.dialogue, outcome, error_class, messages)

    async def _running_bind_failure(self, jobs, starting, dialogue, original: BaseException):
        try:
            return await self._finish(jobs, starting, dialogue, TurnTerminalOutcome.UNKNOWN, "CODEX_AMBIGUOUS", ())
    ...
    async def _finish(self, jobs, job, dialogue, outcome, error_class, messages):
        output = b""
        if messages:
            try:
        ...
        if output:
            ...
            output_id = self._make_id("output")
        try:
