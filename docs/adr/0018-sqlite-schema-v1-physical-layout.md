# ADR-0018 — Exact SQLite schema-v1 physical layout
...
ADR-0017 freezes the P2.1 storage-kernel behavior. This record removes executor discretion from the physical v1 table/column/index layout used by P2 repositories in later slices.

The SQL below is architectural authority. Equivalent reformatting in Python constants is allowed; column names, constraints, foreign-key actions, allowed states and index names/columns are not executor choices.

## Migration identity

- `SCHEMA_VERSION = 1`
- migration ID: `0001_initial_state`
- migration DDL SHA-256 is the SHA-256 of the implementation's single canonical UTF-8 v1 DDL constant containing exactly the statements below in this order. The stored migration row must equal the code's computed canonical hash on every reopen.
- `PRAGMA user_version = 1` is set only inside the same successful migration transaction after the DDL and migration-history insert are ready to commit.
...
Ignoring SQLite internal objects whose names start `sqlite_`, schema version 1 has exactly these user tables:

`schema_migrations`, `controller_runtime`, `settings`, `dialogues`, `turn_jobs`, `transient_payloads`, `delivery_segments`, `ingress_updates`, `callback_actions`, `approvals`, `deletion_tombstones`, `errors`.

The explicit non-auto index set is exactly:
...
SQLite-generated primary-key/unique autoindexes are not part of the explicit index-name set and must not be treated as drift.

## Security/content boundary

No schema-v1 table other than `transient_payloads.content` contains a prompt/response/content BLOB/TEXT column. There is no raw Telegram update JSON, callback token, command output, environment, stderr, auth, cookie or secret column. Hash columns store only hashes; error columns store sanitized classes/fingerprints only.

## Scope boundary

This ADR defines physical layout only. P2.1 does not seed singleton rows and does not implement repository/business state transitions, retention deletion, hard-delete finalization or any production DB migration beyond empty version 0 -> version 1.
