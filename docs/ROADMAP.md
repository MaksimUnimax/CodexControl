...
- [DONE] **P7.C3 — configured persistent profile + isolated state-root runtime authority.** Accepted `f76a32b2d18600fcf7ace6b9aa24067238d6dec7`, tree `13eea89362694da594bb2b717c037980b0df446f`. ADR-0045 supersedes only the later mistaken interpretation that the persistent `CODEX_HOME` itself must be exclusive to CodexControl; C3's isolated-root/runtime-manager ownership remains accepted.

- [DONE] **P7.C4 — confirmed cleanup + DELETE_UNKNOWN local containment.** Accepted final `df161566cab5f8fa7ccf70f94379c78a9fb02ffe`, tree `be2ea7a5eead9b2d61039a2d98ec4d2b80047526`. Schema v4 adds bounded UNKNOWN containment metadata; confirmed finalization requires CodexControl-local reservation/quiescence, crash-resumable isolated reset and persistent exact-thread gate; UNKNOWN remains official UNKNOWN and quarantined. ADR-0045 clarifies that unrelated Codex processes sharing the persistent home are never stopped by this reservation.
...
- [NEXT / REAL-EFFECT ONE-SHOT, SHARED-HOME CORRECTED] **P7.C6 — renewed isolated real T3 + corrected hard-delete acceptance.** Base contract: `docs/evidence/p7c6/P7C6_ARCHITECT_EXECUTION_CONTRACT_2026-09-10.md`; binding correction: `docs/evidence/p7c6/P7C6_ARCHITECT_CONTRACT_CORRECTION_SHARED_HOME_2026-09-10.md`.

  C6 shall use the existing authenticated `/root/.codex_second` as a `SHARED_AUTHENTICATED` persistent home. Other live Codex processes using that same `CODEX_HOME` are allowed and are not blockers by themselves. They must not be killed, stopped or signaled for C6.

  C6 may use exactly one new disposable real thread and one official `thread/delete`. It must create a fresh isolated state root and separate run-owned controller DB, prove that no unrelated process uses those exact protected boundaries, exercise real persisted multi-turn across runtime restart, one approval ALLOW path, one interrupt, pre-delete target-specific physical observation, then the complete C2/C3/C4 application delete path.

  PASS requires exact real `DELETE_CONFIRMED`, final local `DELETED`/tombstone, zero target thread-ID residual, zero known synthetic marker residual across measured dialogue-bearing persistent/isolated families, protected-boundary mount/alias preflight PASS and green ordinary regression. Shared-home background changes are not themselves failures unless they create a concrete target/proof safety error. `DELETE_UNKNOWN`, target residual, inconclusive proof or isolated/controller boundary conflict blocks acceptance and is never retried automatically.

  Previous C6 stops for dedicated-home/process-owner authority occurred before any authenticated business RPC and consumed none of the one-shot real-effect budget.
